...
if [ ${#MISSING_DEPS[@]} -ne 0 ] || [ ${#MISSING_DEV_DEPS[@]} -ne 0 ]; then
    ...
    if command -v apt &> /dev/null; then
        ...
        for dep in "${MISSING_DEV_DEPS[@]}"; do
            case $dep in
                "gtk3-dev")
                    INSTALL_PACKAGES+=("libgtk-3-dev" "libayatana-appindicator3-dev")
                    ;;
            esac
        done
        ...
        for dep in "${MISSING_DEV_DEPS[@]}"; do
            case $dep in
                "gtk3-dev")
                    INSTALL_PACKAGES+=("gtk3-devel" "libayatana-appindicator-gtk3-devel")
                    ;;
            esac
        done
        ...
        for dep in "${MISSING_DEV_DEPS[@]}"; do
            case $dep in
                "gtk3-dev")
                    INSTALL_PACKAGES+=("gtk3" "libayatana-appindicator")
                    ;;
            esac
        done
